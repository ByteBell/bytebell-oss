...
`context.md`. The orchestrator (`pipeline/run.ts`) dispatches to exactly one
strategy per job.
...
- **`flat-folder/`** — active. v2 strategy: clone → scan → big-file split →
  ...
  `context.md` for the 7-phase pipeline.
- **`basic-file-analysis/`** — archived. v1 strategy preserved as a
  `.archived` file. Not compiled, not exported.

Adding a new strategy means: new subfolder + new `context.md` + new
