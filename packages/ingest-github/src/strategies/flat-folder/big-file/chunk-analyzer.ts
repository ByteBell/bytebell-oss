...
export async function analyzeChunk(chunk: FileChunk): Promise<ChunkAnalysisResult> {
  ...
  try {
    const response = await askJsonLLM<Record<string, unknown>>(systemPrompt, userPrompt);
